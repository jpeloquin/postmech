...
class MechanicalTest(object):
    ...
    def __init__(self, ssfile=None, imagelist=None, dir_vic2d=None):
        ...
        if self.imagepaths is not None:
            ...
            self.tab_images = tabulate_images(imdir, vic2d_dir=dir_vic2d)
            # Image times
            self.imagetimes = self.tab_images["Time (s)"]
            # Image lookup table
            imageids = [image_id(f) for f in self.imagepaths]
        ...
        if dir_vic2d is not None:
            ...
            self.strainfields = p.map(vic2d.read_strain_components, vic2dfiles)
            p.close()
            p.join()

            ## Ignore empty strainfields
            self.strainfields = [a for a in self.strainfields if len(a["exx"]) != 0]

            csvnames = (os.path.basename(f) for f in vic2dfiles)
            fieldtimes = [image_time(nm) for nm in csvnames]
    ...
    def from_json(cls, pth_json):
        ...
        # location)
        imagelist = [os.path.join(datadir, fp) for fp in testdesc["images"]]
        # Vic-2D folder
        vic2dfolder = None
        if not pd.isnull(testdesc["vic2d_folder"]):
            vic2dfolder = os.path.join(datadir, testdesc["vic2d_folder"])
        ## Create object
        self = cls(ssfile=pth_ss, imagelist=imagelist, dir_vic2d=vic2dfolder)
        return self
    ...
    def image_at(self, t):
        ...
        imname = "cam{}_{}_{:.3f}".format(
            r["Camera ID"], r["Frame ID"], r["Timestamp (s)"]
        )
...
def first_crossing(v, threshold, to="right"):
    ...
    if direction == -1 and v[-1] >= threshold:
        ...
        idx = next(
            i
            for i, x in zip(np.arange(len(v))[::direction], v[::direction])
            if x > threshold
        )
    return idx
...
def key_stress_pts(test):
    ...
    # Get image data
    imlist = [os.path.basename(s) for s in list_images(test.image_dir)]
    ...
    out["peak stress"] = next(
        nm for nm, t in zip(imlist, imtimes) if t > df["Time (s)"][idx_peak]
    )
    ...
    for f in resfrac:
        ...
        idx = first_crossing(
            df["Stress (Pa)"].values, threshold=f * peak_stress, to="left"
        )
        idx_res.append(idx)
        ...
        if idx_res[-1] is not None:
            ...
            imname = next((nm for t, nm in zip(imtimes, imlist) if t > tc), None)
            out[key] = imname
        else:
            out[key] = None
    ...
    ydiv = 1e6
    ax.plot(df["Stretch Ratio"], df["Stress (Pa)"] / ydiv, color="k")
    plt.tick_params(axis="both", which="major", labelsize=10)
    (ln_peak,) = ax.plot(
        df.loc[idx_peak]["Stretch Ratio"],
    ...
    for i, f in enumerate(resfrac):
        if idx_res[i] is not None:
            (ln_res,) = ax.plot(
                df.loc[idx_res[i]]["Stretch Ratio"],
            ...
            legend_labels.append("{}% strength".format(int(round(f * 100))))
    ax.legend(
        [ln_peak] + lns_res,
        legend_labels,
        loc="upper right",
...
def tabulate_stress_strain(spcdir, data, areapath, lengthpath, imdir=None):
    ...
    length = ref_length.value.magnitude + (
        data["Position (m)"] - data["Position (m)"][0]
    )
...
def xlim_strain(sstable):
    ...
    if sstable["Stress (Pa)"].iloc[-1] > thresh:
        ...
        idx = next(i for i in sstable.index[::-1] if sstable["Stress (Pa)"][i] > thresh)
...
def plot_stress_strain(sstable):
    ...
    ax.plot(sstable["Stretch Ratio"], sstable["Stress (Pa)"] / 1e6, color="k")
...
def ramp_data(test):
    ...
    image_tab = tabulate_images(test.image_dir, test.stress_strain_file, test.vic2d_dir)
